...
QCoro::Task<std::expected<void, QString>> RatingList::import(const QString &name, const QUrl &url)
{
    ...
    connect(reply, &QNetworkReply::downloadProgress, this, [this](qint64 received, qint64 total) {
        const auto progress = 100. * (received / total);
        qDebug() << progress;
        Q_EMIT statusChanged(i18nc("@info:status", "Downloading file (%1 %)", progress));
    });
...
std::expected<int, QString> RatingList::readPlayers(QTextStream *stream)
{
    ...
    auto addPlayers = [&db, &lists, &names, &ids, &federations, &stdRatings, &rpdRatings, &btzRatings]() -> std::expected<void, QString> {
        QSqlQuery query(*db);
